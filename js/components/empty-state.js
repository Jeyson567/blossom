const ICONS = {
  ...
  activity: '<path d="M13 3c-4.97 0-9 4.03-9 9H1l3.89 3.89.07.14L9 12H6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42A8.954 8.954 0 0013 21c4.97 0 9-4.03 9-9s-4.03-9-9-9zm-1 5v5l4.28 2.54.72-1.21-3.5-2.08V8H12z"/>',
  default: '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/>'
};

/**
 * Estado vacío profesional con icono y botón de acción opcional.
 */
export function renderEmptyState({ icon = 'default', title, message = '', actionLabel, actionId }) {
  const path = ICONS[icon] || ICONS.default;
  const actionBtn = actionLabel && actionId
    ? `<button type="button" class="btn btn-primary empty-state-action" id="${actionId}">
  ...
  return `
    <div class="empty-state empty-state-pro">
      <div class="empty-state-icon">
        <svg viewBox="0 0 24 24" fill="currentColor" width="56" height="56">${path}</svg>
      </div>
...
/** Mensaje amigable para errores de carga (sin referencias técnicas a Firestore). */
export function friendlyLoadError(error) {
  const code = error?.code || '';
  if (code === 'permission-denied') {
    return 'No tiene permisos para ver esta información. Contacte al administrador del sistema.';
  }
...
export function bindEmptyAction(actionId, handler) {
  document.getElementById(actionId)?.addEventListener('click', handler);
}
